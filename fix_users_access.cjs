...
const functionsToAdd = `
        window.deleteUser = function(index) {
            if(confirm("Voulez-vous vraiment supprimer cet utilisateur ?")) {
                ...
                renderAdmin(document.getElementById('content'), 'admin-users');
        ...
        window.submitAddUser = function() {
            ...
            const profil = document.getElementById('new-user-profil').value;
            const agence = document.getElementById('new-user-agence').value;
            ...
            MOCK.admins.push({ nom, email, profil, agence, statut: 'Actif', dashboards: 0 });
            renderAdmin(document.getElementById('content'), 'admin-users');
            showToast("Utilisateur ajouté avec succès");
        };

        window.toggleModuleRole = function(moduleId, role) {
            const module = APP.modules.find(m => m.id === moduleId);
            if(module) {
                if(!module.roles) module.roles = [];
                if(module.roles.includes(role)) {
                    module.roles = module.roles.filter(r => r !== role);
                } else {
                    module.roles.push(role);
                }
                renderAdmin(document.getElementById('content'), 'admin-access');
                buildSidebar();
...
html = html.replace(/window\.switchTab =/, functionsToAdd);

// Now update renderAdmin
const renderAdminRegex = /function renderAdmin\(container, activeTab = 'admin-users'\) \{[\s\S]*?(?=function renderChatbot)/;
                    ...
                    <div style="flex: 1; overflow-y: auto;">
                        ...
                        <div id="admin-users" class="\${activeTab === 'admin-users' ? '' : 'hidden'}">
                            <div class="card">
                                <div class="card-header">
                                    ...
                                    <button class="btn btn-primary" onclick="toggleAddUserForm()">+ Nouvel Utilisateur</button>
                                </div>
                                
                                <div id="add-user-form" class="hidden" style="padding: 20px; background: var(--light-bg); border-radius: 8px; margin-bottom: 20px;">
                                    <h4 style="margin-bottom: 16px; color: var(--dark-teal); font-weight: 600;">Ajouter un nouvel utilisateur</h4>
                                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 16px;">
                                        <div>
                                            <label style="display:block; margin-bottom:8px; font-size:13px; font-weight:600;">Nom complet</label>
                                            <input type="text" id="new-user-nom" placeholder="Ex: Ahmed Alaoui" style="width:100%; padding:10px; border:1px solid var(--sec-bg); border-radius:6px;">
                                        </div>
                                        <div>
                                            <label style="display:block; margin-bottom:8px; font-size:13px; font-weight:600;">Email</label>
                                            <input type="email" id="new-user-email" placeholder="Ex: ahmed.alaoui@saham.ma" style="width:100%; padding:10px; border:1px solid var(--sec-bg); border-radius:6px;">
                                        </div>
                                        <div>
                                            <label style="display:block; margin-bottom:8px; font-size:13px; font-weight:600;">Profil Technique</label>
                                            <select id="new-user-profil" style="width:100%; padding:10px; border:1px solid var(--sec-bg); border-radius:6px;">
                                                <option value="Admin IT">Admin IT</option>
                                                <option value="DG">Directeur Général (DG)</option>
                                                <option value="DR">Directeur Régional (DR)</option>
                                                <option value="CA">Chargé d'Affaires (CA)</option>
                                                <option value="AR">Analyste Risque (AR)</option>
                                            </select>
                                        </div>
                                        <div>
                                            <label style="display:block; margin-bottom:8px; font-size:13px; font-weight:600;">Agence / DR</label>
                                            <input type="text" id="new-user-agence" placeholder="Ex: Siège Casa" style="width:100%; padding:10px; border:1px solid var(--sec-bg); border-radius:6px;">
                                        </div>
                                    </div>
                                    <div style="display: flex; gap: 12px; justify-content: flex-end;">
                                        <button class="btn btn-secondary" onclick="toggleAddUserForm()">Annuler</button>
                                        <button class="btn btn-primary" onclick="submitAddUser()">Enregistrer</button>
                                    </div>
                                </div>
                        ...
                        <div id="admin-access" class="\${activeTab === 'admin-access' ? '' : 'hidden'}">
                            <div class="card" style="padding:32px;">
                                <h3 class="chart-title">Matrice des Permissions</h3>
                                <p style="color:var(--slate-500); margin-bottom:24px;">Configurez quels rôles ont accès à quels modules. Les modifications sont appliquées immédiatement.</p>
                                
                                <div class="table-responsive" style="border: 1px solid var(--sec-bg); border-radius: 8px;">
                                    <table style="margin:0;">
                                        <thead style="background: var(--light-bg);">
                                            <tr>
                                                <th style="font-weight:700;">Module</th>
                                        ...
                                        <tbody>
                                            \${APP.modules.map(m => {
                                                if (m.isGroup) {
                                                    let rows = \`<tr><td colspan="6" style="background:var(--light-bg); font-weight:700; font-size:11px; text-transform:uppercase; color:var(--slate-500);">\${m.name}</td></tr>\`;
                                                    m.subItems.forEach(sub => {
                                                        const roles = sub.roles || m.roles || [];
                                                        rows += \`<tr>
                                                            <td style="padding-left:24px;">\${sub.name}</td>
                                                            <td style="text-align:center;"><input type="checkbox" \${roles.includes('DG') ? 'checked' : ''} onchange="toggleModuleRole('\${sub.id}', 'DG')"></td>
                                                            <td style="text-align:center;"><input type="checkbox" \${roles.includes('DR') ? 'checked' : ''} onchange="toggleModuleRole('\${sub.id}', 'DR')"></td>
                                                            <td style="text-align:center;"><input type="checkbox" \${roles.includes('CA') ? 'checked' : ''} onchange="toggleModuleRole('\${sub.id}', 'CA')"></td>
                                                            <td style="text-align:center;"><input type="checkbox" \${roles.includes('AR') ? 'checked' : ''} onchange="toggleModuleRole('\${sub.id}', 'AR')"></td>
                                                            <td style="text-align:center;"><input type="checkbox" \${roles.includes('Admin') ? 'checked' : ''} onchange="toggleModuleRole('\${sub.id}', 'Admin')"></td>
                                                        </tr>\`;
                                                    });
                                                    return rows;
                                                } else {
                                                    ...
                                                    return \`<tr>
                                                        ...
                                                        <td style="text-align:center;"><input type="checkbox" \${roles.includes('DG') ? 'checked' : ''} onchange="toggleModuleRole('\${m.id}', 'DG')"></td>
                                                        <td style="text-align:center;"><input type="checkbox" \${roles.includes('DR') ? 'checked' : ''} onchange="toggleModuleRole('\${m.id}', 'DR')"></td>
                                                        <td style="text-align:center;"><input type="checkbox" \${roles.includes('CA') ? 'checked' : ''} onchange="toggleModuleRole('\${m.id}', 'CA')"></td>
                                                        <td style="text-align:center;"><input type="checkbox" \${roles.includes('AR') ? 'checked' : ''} onchange="toggleModuleRole('\${m.id}', 'AR')"></td>
                                                        <td style="text-align:center;"><input type="checkbox" \${roles.includes('Admin') ? 'checked' : ''} onchange="toggleModuleRole('\${m.id}', 'Admin')"></td>
                                                    </tr>\`;
                                                }
                                            }).join('')}
                                        </tbody>
